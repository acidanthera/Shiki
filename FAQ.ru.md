...
- _С какими версиями OS X он работает?_  
Он работает, начиная от 10.9 и выше. Совместимость со всеми промежуточными версиями ОС не проверялась. Обратите внимание, что Shiki работает только со сравнительно новыми версиями iTunes, например, тестирование проводилось с 12.3.
...
- _Какие видеокарты (GPU) поддерживаются с недочётами?_  
   - Видеокарты, использующие фреймбуферы Azul (например, HD 4400, HD4600), при использовании полноценного завода в режиме единственного GPU (без дискретной видеокарты) приводят к зависанию системы и потому не поддерживаются. Это связано с некорректно работающим HDCP на этих картах. Проблема не имеет отношения к Shiki и происходит независимо от iTunes и QuickTime. 
   - Некоторые карточки (например, HD 4000) иногда приводят к падениям iTunes/Quicktime при воспроизведении HD фильмов по неуточнённым причинам. Предположительно это ошибка Apple при работе с HDCP. Переустановка системы или сброс конфигурации DRM (см. ниже) обычно помогают решить проблему. Другим способом является использование руководства для неподдерживаемых процессоров.
...
В данный момент не полностью. Утилиты и [патчер ядра](https://github.com/vit9696/AppleALC/tree/master/Common) доступны для пользования.
...
- _Как мне изменить настройки аппаратного декодирования?_  
...
- Проблему можно решить без Shiki, если у вас имеется:
  ...
  Это решение плохо изучено, но работает у нескольких людей. Кекст не нужен.
...
_Спасибо: 07151129, Andrey1970, Сашко666, chrome, family1232009, garcon, iDark Soul, igork, m-dudarev, Mieze, tatur_sn, Quadie, и другим._